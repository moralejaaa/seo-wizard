...
import { Upload, Zap, Loader2, CheckCircle, FileSpreadsheet, FileArchive, Copy, Download } from 'lucide-react';
...
export default function SEOWizard() {
  ...
  const downloadExcel = () => {
    // Añadimos el BOM para que Excel reconozca acentos en español
    const BOM = "\uFEFF";
    ...
    for (const file of files) {
      ...
      try {
        ...
        const resp = await fetch(apiUrl, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Analiza la imagen y responde solo en formato JSON: {\"fileName\": \"nombre-archivo-seo\", \"altText\": \"descripcion alt en español con palabras clave\"}" },
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 font-sans">
      <nav className="max-w-5xl mx-auto flex justify-between items-center mb-12 py-4 border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          {results.length > 0 && (
            <>
              <button onClick={downloadExcel} className="cursor-pointer bg-emerald-600 hover:bg-emerald-700 px-3 py-2 rounded-xl text-[10px] font-bold uppercase flex items-center gap-1 shadow-lg transition-colors">
                <FileSpreadsheet className="w-3 h-3" /> EXCEL
              </button>
              <button onClick={downloadZIP} className="cursor-pointer bg-violet-600 hover:bg-violet-700 px-3 py-2 rounded-xl text-[10px] font-bold uppercase flex items-center gap-1 shadow-lg transition-colors">
      ...
      <main className="max-w-2xl mx-auto">
        <div className="border-2 border-dashed border-violet-500/20 bg-black rounded-[2rem] p-16 text-center hover:border-violet-500 transition-all mb-8 group">
          <label className="cursor-pointer block">
            {loading ? <Loader2 className="w-10 h-10 text-violet-500 animate-spin mx-auto" /> : <Upload className="w-10 h-10 text-violet-500 mx-auto mb-4 group-hover:scale-110 transition-transform" />}
        ...
        <div className="space-y-4">
          {results.map(res => (
            <div key={res.id} className="bg-white/[0.03] p-5 rounded-3xl flex flex-col md:flex-row items-center gap-5 border border-white/5 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="flex-1 text-left min-w-0 w-full">
                ...
                <p className="text-[13px] text-gray-300 italic leading-relaxed font-medium">"{res.altText}"</p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => copyToClipboard(res.altText)} className="cursor-pointer p-2 hover:bg-white/10 rounded-lg transition-colors text-gray-400 hover:text-white" title="Copiar Alt Text">
                  <Copy className="w-4 h-4" />
                </button>
                <button onClick={() => downloadSingle(res)} className="cursor-pointer p-2 hover:bg-white/10 rounded-lg transition-colors text-gray-400 hover:text-white" title="Descargar Imagen">
